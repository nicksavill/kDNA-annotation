...
def get_cassettes(gRNA_peaks, mO_forward_scores, up, mO_reverse_scores, 
    forward_search_min, forward_search_max, reverse_search_min, reverse_search_max, 
    min_forward_score, min_reverse_score, repeat_len, minicircles):
    cassettes = []
...
def plot(mO, minicircles, cassettes, gRNA_peaks, mO_forward_scores, up, mO_reverse_scores, 
    forward_search_min, forward_search_max, reverse_search_min, reverse_search_max, 
    min_forward_score, min_reverse_score, repeat_len, mO_gRNA_scores, min_gRNA_score):
    ...
    for _, c in cassettes.query('mO_name == @mO_name').iterrows():
        axes.plot([c['forward_start'], c['reverse_end']], [0, 0], color='Orange')
    ...
    axes.axhline(min_gRNA_score, color='b', ls=':', label='gRNA threshold')
    axes.axhline(min_forward_score, color='k', ls='--', label='forward repeat threshold')
    axes.axhline(min_reverse_score, color='r', ls='--', label='reverse repeat threshold')
...
def main(config_file='config.yaml'):
    ...
    ################################### IDENTIFY CASSETTES #######################################
    # identify cassettes based on gRNA and repeat scores
    cassettes = get_cassettes(*(mO_scores[:-2]), minicircles)
    ...
    for mO in plot_mOs:
        plot(mO, minicircles, cassettes, *mO_scores)
